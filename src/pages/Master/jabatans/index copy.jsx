...
const Advanced = () => {
  ...
  // 🔹 Save (Tambah / Edit) - UPDATED dengan validasi skor
  const handleSave = async () => {
    ...
    // Validasi skor harus diisi dan angka positif
    if (form.skor === "" || form.skor === null) {
      MySwal.fire({
        icon: "warning",
        title: "Skor wajib diisi!",
      });
      return;
    }

    if (parseInt(form.skor) < 0) {
      MySwal.fire({
        icon: "warning",
    ...
    try {
      if (editId) {
        await axiosAuth.put(`/jabatans/${editId}`, form);
        ...
        await axiosAuth.post("/jabatans", form);
  ...
  const handleEdit = (jabatan) => {
    ...
    setForm({
      ...
      skor: jabatan.skor || "", // Ambil nilai skor, default empty string
    });
    setShowModal(true);
  };
  ...
  return (
    <>
      ...
      {/* Modal Tambah / Edit - DITAMBAH INPUT SKOR */}
      <Modal show={showModal} onHide={handleClose}>
        ...
        <Modal.Body>
          <Form>
            ...
            {/* <Form.Group className="mb-3">
              <Form.Label>Nominal Denda</Form.Label>
              <Form.Control
                type="number"
                value={form.nominal_denda}
                onChange={(e) => setForm({ ...form, nominal_denda: e.target.value })}
                min="0"
            ...
            {/* <Form.Group className="mb-3">
              <Form.Label>
                Skor Jabatan <span className="text-danger">*</span>
              ...
              <Form.Control
                ...
                min="0"
                placeholder="Masukkan skor jabatan"
                required
              />
              <Form.Text className="text-muted">
                Skor awal yang akan diberikan kepada talent setiap tahunnya
              </Form.Text>
            </Form.Group> */}
          </Form>
        </Modal.Body>
