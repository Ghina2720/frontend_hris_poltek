...
// images
import logoSm from "@/assets/images/logo-syntax.png";
import logoDark from "@/assets/images/logo-syntax.png";
import logoDark2 from "@/assets/images/logo-syntax.png";
import logoLight from "@/assets/images/logo-light.png";
import logoLight2 from "@/assets/images/logo-syntax.png";
...
const MenuItemWithChildren = ({
  ...
  return <>
            <li className={classNames("menu-item", "nav-link-ref", {
      "menuitem-active": open
    })}>
                <Link to="#" onClick={toggleMenuItem} data-menu-key={item.key} aria-expanded={open} className={classNames("menu-link justify-content-between", linkClassName, {
...
const MenuItemLink = ({
  ...
  return <Link to={item.url} target={item.target} className={classNames("menu-link nav-link-ref justify-content-between", className)} data-menu-key={item.key}>
            <span className="menu-text"> {item.label} </span>
...
const MainMenu = ({
  ...
  return <>
            {activeMenuItems && <div className="app-menu">
            ...
            return <div key={key} className={classNames("twocolumn-menu-item", {
              "d-block": activeParent
                  ...
                  return <React.Fragment key={idx}>
                                                        {item.children ? <MenuItemWithChildren item={item} toggleMenu={toggleMenu} subMenuClassNames="sub-menu" activeMenuItems={activeMenuItems} linkClassName="" /> : <MenuItem item={item} linkClassName="" className={activeMenuItems.includes(item.key) ? "menuitem-active" : ""} />}
                                                    </React.Fragment>;
                })}
                                        </ul>
                                        {/* </div> */}
